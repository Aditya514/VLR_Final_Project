...
def full_train_of_nn_with_hps(net, loss_fn, train_dataset, hps_gen, epochs, device=None, train_attack=None,
    ...
    if hps_gen.size() > 1 or (hps_gen.size() == 1 and early_stop):
        while True:
            ...
            trainer.train_nn(False,net, nn_optimizer, loss_fn, train_dl, epochs, device=device, attack=attack_obj,
    ...
    if full_train or (hps_gen.size() == 1 and not early_stop):
        ...
        trainer.train_nn(True,net, nn_optimizer, loss_fn, full_train_dl, epochs, device=device, attack=attack_obj,
